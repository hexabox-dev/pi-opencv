...
def sonar(GPIO_TRIGGER,GPIO_ECHO):
      ...
      # no wait forever: max 50ms and 100ms
      while GPIO.input(GPIO_ECHO)==0 and time.time()<begin+0.05:
            start = time.time()
     
      while GPIO.input(GPIO_ECHO)==1 and time.time()<begin+0.10:
            stop = time.time()
...
for image in camera.capture_continuous(rawCapture, format="bgr", use_video_port=True):
      ...
      hsv1 = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV)     # convert BGR to HSV, but not used?
      mask_red=segment_colour(frame)      #masking red the frame
      loct,area=find_blob(mask_red)
      x,y,w,h=loct      # location of blob
      ...
      flag=0        # direction: turn left or right
      initial=400   # area comparison thresholds
      initial2=6700
